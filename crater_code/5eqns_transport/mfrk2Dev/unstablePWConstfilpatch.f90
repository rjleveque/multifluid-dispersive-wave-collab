...
recursive subroutine filrecur(level,nvar,valbig,aux,naux,t,mitot,mjtot, &
  ...
  integer :: ifoo,jfoo
  real(kind=8) :: box(3,3),rminInd
  integer :: indexItouse,indexJtouse
  logical :: noSlope
  ...
  if (.not.set) then
     ...
     do j_fine  = 1,mjtot_patch
        ...
        do i_fine = 1,mitot_patch
           ...
           do jfoo = 1,3
           do ifoo = 1,3
              box(ifoo,jfoo) = valcrse(coarse_index(6,i_coarse-2+ifoo,j_coarse-2+jfoo))
           end do
           end do
           

            noSlope =  (maxval(box) -  minval(box) > .2) 
            !noSlope = .false.
            !noSlope = .true.
              ...
              do n=1,nvar
               ...
               if (.not. noSlope) then
                 ...
                 ! find cell with closest indicator for pw const assignment
                 indexItouse = 2
                 indexJtouse = 2
                 rminInd = 2.d0
                 do jfoo = 1,3
                 do ifoo = 1,3
                 if (valcrse(coarse_index(6,i_coarse-2+ifoo,j_coarse-2+jfoo)) .lt. rminInd) then
                     indexItouse = i_coarse-2+ifoo
                     indexJtouse = j_coarse-2+jfoo
                     rminInd = valcrse(coarse_index(6,i_coarse-2+ifoo,j_coarse-2+jfoo))
                 endif

                 end do
                 end do
                 valbig(n,i_fine+nrowst-1,j_fine+ncolst-1) = valcrse(coarse_index(n,indexItouse,indexJtouse))
               endif
            end do
