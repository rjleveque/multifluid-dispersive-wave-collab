...
c     # solid wall
      do m=1,meqn
         do ibc=1,mbc
c           # KMS oct23, 2025
c            ql(1-ibc,m) = ql(ibc,m)
c            qr(1-ibc,m) = qr(ibc,m)
            ql(1-ibc,m) = qr(ibc,m)
            qr(1-ibc,m) = ql(ibc,m)
            enddo
         enddo
...
c     # solid wall
      do m=1,meqn
         do ibc=1,mbc
c            ql(mx+ibc,m) = ql(mx+1-ibc,m)
c            qr(mx+ibc,m) = qr(mx+1-ibc,m)
            ql(mx+ibc,m) = qr(mx+1-ibc,m)
            qr(mx+ibc,m) = ql(mx+1-ibc,m)
            enddo
         enddo
c
c     # negate the normal velocity:
...
c     # solid wall
      do m=1,meqn
         do jbc=1,mbc
c            ql(1-jbc,m) = ql(jbc,m)
c            qr(1-jbc,m) = qr(jbc,m)
            ql(1-jbc,m) = qr(jbc,m)
            qr(1-jbc,m) = ql(jbc,m)
            enddo
         enddo
...
c     # solid wall
      do m=1,meqn
         do jbc=1,mbc
c            ql(mx+jbc,m) = ql(mx+1-jbc,m)
c            qr(mx+jbc,m) = qr(mx+1-jbc,m)
            ql(mx+jbc,m) = qr(mx+1-jbc,m)
            qr(mx+jbc,m) = ql(mx+1-jbc,m)
            enddo
         enddo
c
c     # negate the normal velocity:
